...
private:
  ...
  void DeclareParams(){
      ...
      declare_parameter("historyKeyframeFitnessScore", 0.2);
      ...
      declare_parameter("odometryCurrentScanLeafSize", 0.2);
  ...
  void initialpose_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr pose_msg)
  {
      ...
      // if(currentRelocStatus == Initialized){
    ...
    CloudPtr eigenToPointCloud(const Eigen::MatrixXf &eigen_matrix) {
  ...
  void ICPLocalize_test(){
    ...
    transformTobeMapped[0] = latest_odom_pose_struct.roll;
    transformTobeMapped[1] = latest_odom_pose_struct.pitch;
    transformTobeMapped[2] = latest_odom_pose_struct.yaw;
    transformTobeMapped[3] = latest_odom_pose_struct.x;
    transformTobeMapped[4] = latest_odom_pose_struct.y;
    transformTobeMapped[5] = latest_odom_pose_struct.z;
 ...
 // Function to create an Eigen::Affine3f transformation from PointTypePose (added by wataru 6/20/2024)
  ...
  float ini[6];// the pose in the world, i.e. the prebuilt map
  float initialposeInMap[6];// the pose in the world, i.e. the prebuilt map
